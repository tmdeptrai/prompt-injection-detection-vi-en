...
def main():
    ...
    # --- MODIFIED: Load multiple metrics for standard classification ---
    if data_args.metric_name is not None:
        ...
        if is_regression:
            ...
            logger.info("Using multilabel F1 for multi-label classification task.")
        else:
            # Load a combination of standard classification metrics
            metric = evaluate.combine(["accuracy", "f1", "precision", "recall"])
            logger.info("Using combined metrics (accuracy, f1, precision, recall) for classification score.")
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            # Standard Single-Label Classification (Your Task)
            preds = np.argmax(preds, axis=1)
            # This computes accuracy, f1, precision, and recall all at once
            result = metric.compute(predictions=preds, references=p.label_ids)
